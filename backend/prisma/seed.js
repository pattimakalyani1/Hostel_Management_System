...
async function main() {
  ...
  let wardenUser;
  if (existingAdmin) {
    wardenUser = await prisma.user.update({
  ...
  for (const rd of roomDefs) {
    const room = await prisma.room.create({
      data: {
        ...
        capacity:    rd.sharingType,   // capacity always equals sharingType
        description: rd.description,
        roomType:    `${rd.sharingType} Sharing`,
        status:      'AVAILABLE'       // will be derived dynamically by API, stored here as initial
      }
    });
  ...
  // ── 4. Beds ────────────────────────────────────────────────────────────
  // Helper: create beds for a room
  async function createBeds(room, bedConfigs) {
    // bedConfigs: [{ number: 'Bed 1', status, maintenanceReason, maintenanceDate }]
    const beds = [];
  ...
  const allocations = [
    ...
    // Room 103 – 2 sharing – ₹6,000  (Bed 2 is maintenance, no student)
    ...
    // Room 201 – 4 sharing – ₹5,000  (Bed 4 is maintenance, 3 students on Beds 1-3)
